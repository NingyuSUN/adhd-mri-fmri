...
- **101→98的差异，已逐列核实**：直接比对了试点输出的101列表头和 `volumes_NOT_TRAINING_READY.npz` 里实际使用的98列，精确差集只有3列：`total intracranial`（即TIV，被单独抽出作为 `total_intracranial_mm3` 字段，不算进区域体积列表）、`left cerebral cortex`、`right cerebral cortex`（左右大脑半球皮层的整体体积——这是 SynthSeg 给出的粗粒度"全皮层"汇总量，会被后面68个 Desikan-Killiany 细分皮层区域重复覆盖，所以被剔除，避免整体皮层体积和它的68个子区域体积同时出现在特征里造成冗余/共线性）。**没有其他区域被增删或改名**，98列和101列里剩下的98个名字逐一比对完全一致。
