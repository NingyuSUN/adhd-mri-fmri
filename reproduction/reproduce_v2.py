...
def unit(source,out,job):
    ...
    if (dest/'reproduction_complete.json').exists():
        return verify_unit(dest,expected_job=job,protocol_hash=digest(out/'protocol_v2.json'))
...
def main():
    ...
    reference_hashes=json.loads((HERE/'reference_manifest.json').read_text())['files']
    verify_manifest(original,reference_hashes)
    timeseries=source/'runs/temporal_cache_20260907_113751/timeseries.npz'
    verified_timeseries=json.loads((timeseries.parent/'verification.json').read_text())['cache_sha256']
    if digest(timeseries)!=verified_timeseries:raise ValueError('time-series input hash mismatch')
    ...
    manifest=dict(original_protocol_sha256=digest(original/'protocol_v2.json'),legacy_manifest_sha256=digest(HERE/'legacy_manifest.json'),
                  ...
                  completion_sha256=digest(HERE/'completion.py'),reference_manifest_sha256=digest(HERE/'reference_manifest.json'),
                  original_reference_hashes=reference_hashes,timeseries_sha256=verified_timeseries,
    ...
    covariance_reused=(out/'covariances.npy').exists()
    ...
    write_json(out/'covariance_comparison.json',dict(max_abs_difference=maxdiff,sha256_identical=digest(out/'covariances.npy')==digest(original/'covariances.npy'),recomputed_from_timeseries=not covariance_reused,cache_reused_this_invocation=covariance_reused,timeseries_sha256=verified_timeseries))
    del fresh,saved
    ...
    try:
        ...
        def summarize():
            if len(results)==66:
                ...
                for fw in ['cv','loso']:
                    for co in COHORTS:
                        if pd.read_csv(out/fw/co/'summary.csv').shape[0]!=17:raise ValueError('incomplete summary')
        finalize_report(out,report,summarize)
